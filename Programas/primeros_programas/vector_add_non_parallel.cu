...
int main(){
    ...
    // Llamamos al kernel de CUDA (1 bloque, 1 thread por bloque).
    vector_add<<<1,1>>>(cuda_out, cuda_a, cuda_b, N);
    //Esperamos a que el GPU finalice todo su trabajo.
    ...
    free(b);
    free(out);
    //Sugiero ver el comando: nvprof ./vector_add_non_parallel
}
